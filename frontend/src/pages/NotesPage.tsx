...
import { EmptyState, formatDate, Modal, Spinner, SyncBadge } from '../components/ui'
...
type Clipboard = { kind: 'note' | 'folder'; id: string } | null
...
export default function NotesPage() {
  ...
  const [propsTarget, setPropsTarget] = useState<{ kind: 'note' | 'folder'; id: string } | null>(null)
  ...
  const clipboardLabel =
    clipboard?.kind === 'note'
      ? notes[clipboard.id]?.plain.title || 'untitled'
    ...
    <section className="animate-fadeIn flex flex-col" style={{ minHeight: '60vh' }}>
      <div className="glass mb-4 flex flex-col gap-3 p-4 shadow-xl sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full items-center gap-2 sm:w-auto sm:justify-end">
          ...
          {clipboard ? (
            <button
              onClick={() => pasteInto(currentFolder)}
              title={`paste "${clipboardLabel}" here`}
                ...
                ? [
                    {
                      ...
                      icon: Pencil,
                      onSelect: () => {
                        setRenameTarget({ kind: 'folder', id: item.id, name })
                        setRenameValue(name)
                      },
                    },
                    { key: 'cut', label: 'cut', icon: Scissors, onSelect: () => setClipboard({ kind: 'folder', id: item.id }) },
                    {
                      key: 'paste',
                      label: 'paste',
                      icon: ClipboardPaste,
                      hidden: !clipboard || clipboard.id === item.id,
                      onSelect: () => pasteInto(item.id),
                    },
                    {
                      key: 'delete',
                      label: 'delete',
                      icon: Trash2,
                      danger: true,
                      onSelect: () => setConfirmTrash({ collection: 'folders', id: item.id, name }),
                    },
                    {
                      key: 'properties',
                      label: 'properties',
                      icon: Info,
                      onSelect: () => setPropsTarget({ kind: 'folder', id: item.id }),
                    },
                  ]
                : [
                    {
                      ...
                      icon: Pencil,
                      onSelect: () => {
                        setRenameTarget({ kind: 'note', id: item.id, name })
                        setRenameValue(name)
                      },
                    },
                    { key: 'cut', label: 'cut', icon: Scissors, onSelect: () => setClipboard({ kind: 'note', id: item.id }) },
                    {
                      key: 'delete',
                      label: 'delete',
                      icon: Trash2,
                      danger: true,
                      onSelect: () => setConfirmTrash({ collection: 'notes', id: item.id, name }),
                    },
                    {
                      key: 'properties',
                      label: 'properties',
                      icon: Info,
                      onSelect: () => setPropsTarget({ kind: 'note', id: item.id }),
                    },
                  ]
              return (
                <div
      ...
      {renameTarget ? (
        <Modal
          title={`Rename ${renameTarget.kind === 'folder' ? 'category' : 'note'}`}
            ...
            onSubmit={(event) => {
              ...
              if (name) {
                if (renameTarget.kind === 'folder') {
                  useVault.getState().renameFolder(renameTarget.id, name)
                } else {
                  useVault.getState().renameNote(renameTarget.id, name)
                }
              }
      ...
      {propsTarget?.kind === 'note' && notes[propsTarget.id] ? (
        <PropertiesModal
          ...
          rows={[
            ...
            { label: 'edited', value: new Date(notes[propsTarget.id].plain.edited).toLocaleString() },
            { label: 'created', value: formatDate(notes[propsTarget.id].createdAt) },
            { label: 'modified', value: formatDate(notes[propsTarget.id].updatedAt) },
            { label: 'id', value: notes[propsTarget.id].id },
          ]}
          onClose={() => setPropsTarget(null)}
        />
      ) : null}
